...
var iPageHeight_A4 = 1034;	// the real page height of A4
...
PageBreaker.prototype.run = function (){
	...
	for(i = 0; i < document.all.length; i++){	
		...
		if(document.all(i).tagName.toLowerCase()=="table" || document.all(i).tagName.toLowerCase()=="img"){
			...
			if((this.getActualOffsetTop(obj)-iNewPageOffsetTop)+obj.offsetHeight>iPageHeight){
				if(bShowAlert) {
					var objid = "";
					try{
						objid = obj.id;
					...
					this.log(objid+":(obj.offsetTop("+this.getActualOffsetTop(obj)+")-iNewPageOffsetTop("+iNewPageOffsetTop+"))+obj.offsetHeight("+obj.offsetHeight+")>iPageHeight("+iPageHeight+")");
				}
				...
				if((this.getActualOffsetTop(obj)-iNewPageOffsetTop)+obj.offsetHeight>iPageHeight){
					...
					if(document.all(i).tagName.toLowerCase()=="table"){
						...
						iNewPageOffsetTop = this.getActualOffsetTop(obj)+obj.offsetHeight-(obj.offsetHeight % iPageHeight);	// get the remainder
					}
				}else{
	...
	for(var rowidx=0; rowidx<oTable.rows.length; rowidx++){
		...
		for(var colidx=0; colidx<oTr.cells.length; colidx++){
			...
			if((this.getActualOffsetTop(oTr)+oTd.offsetHeight-iNewPageOffsetTop)>iPageHeight && oTd.rowSpan==breakRowSpan){		
				...
				if(oTd.offsetHeight>iPageHeight){
					iNewPageOffsetTop = this.getActualOffsetTop(oTr)+oTd.offsetHeight-(oTd.offsetHeight % iPageHeight);	// 取餘數
				}else{
