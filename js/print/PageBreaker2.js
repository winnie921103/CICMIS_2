...
var iPageHeight_A4 = 1037;	// the real page height of A4 // 原來是 1018
...
PageBreaker.prototype.run = function (){
	...
	iPaperFirstLineOffsetTop = iPaperFirstLineOffsetTop%35;
...
//2. document 內容設定。 
	...
	if(iPaperFirstLineOffsetTop  != 0 ){
		if(this.isOdd)
		{
			...
			div.style.height = (iPaperFirstLineOffsetTop*25+iheaderHeight)-15+"px";
			div.innerHTML = "&nbsp;";
...
//* 針對 tr tag 作的分頁處理
		if(!(document.all(i).tagName.toLowerCase()=="table" || document.all(i).tagName.toLowerCase()=="img" || document.all(i).tagName.toLowerCase()=="td" ||
			 document.all(i).tagName.toLowerCase()=="form" ||document.all(i).tagName.toLowerCase()=="span" )&& 
		...
		{			
			...
			if (next == null || next == undefined)
			{
				...
				if(pageCount == 0){
					...
					total = pageCount*35+countInputTag-iPaperFirstLineOffsetTop+oldiPaperFirstLineOffsetTop;
				}
